...
type UserPage = { users: User[]; page: number; totalPages: number; totalCount: number };

export function TeamManager({ leaders, managers, admins, channels, canManage }: { leaders: UserPage; managers: UserPage; admins: User[]; channels: Channel[]; canManage: boolean }) {
      ...
      {editing && <>
        <div className="flex items-start justify-between border-b p-6">
          <div>
            ...
            <p className="text-sm text-slate-500">{editing.role === "MANAGER" ? "Ekip Lideri" : "Kanal Yöneticisi"} · Kanalları tek tek seçin</p>
          </div>
          <ModalCloseButton onClick={() => setEditing(null)} label="Kapat"/>
        </div>
          ...
          {visibleChannels.length ? visibleChannels.map(channel => {
            ...
            return <button key={channel.id} onClick={() => setSelected(current => { const next = new Set(current); checked ? next.delete(channel.id) : next.add(channel.id); return next; })} className={`flex items-center gap-3 rounded-xl border p-3 text-left ${checked ? "border-teal-500 bg-teal-50 dark:bg-teal-500/10" : "hover:bg-muted"}`}>
              ...
              <span className="min-w-0 flex-1">
                ...
                {assignedToOther && <span className="text-[10px] font-semibold text-amber-600">Başka bir kullanıcıya atanmış</span>}
              </span>
            </button>;
        ...
        <div className="flex justify-end gap-2 border-t p-4">
          ...
          <button disabled={loading} className="btn-primary" onClick={saveAssignments}>{loading ? "Kaydediliyor..." : `${selected.size} kanalı kaydet`}</button>
        </div>
      </>}
    </Modal>

    <Modal open={createOpen} onClose={() => setCreateOpen(false)} className="w-full max-w-md">
      <form onSubmit={createUser} className="space-y-4">
        <div className="flex justify-between">
          <div>
            <h2 className="text-xl font-black">Ekip hesabı ekle</h2>
            <p className="text-sm text-slate-500">Rolü seçip daha sonra kanallarını atayın.</p>
          </div>
          <ModalCloseButton onClick={() => setCreateOpen(false)} label="Kapat"/>
        </div>
        {[["name", "Ad soyad", "text"], ["email", "E-posta", "email"], ["password", "Geçici parola", "password"]].map(field => <input key={field[0]} name={field[0]} type={field[2]} placeholder={field[1]} required minLength={field[0] === "password" ? 8 : undefined} className="h-11 w-full rounded-xl border bg-card px-4"/>)}
        <select name="role" className="h-11 w-full rounded-xl border bg-card px-4"><option value="MANAGER">Ekip Lideri</option><option value="EDITOR">Kanal Yöneticisi</option></select>
        <button disabled={loading} className="btn-primary w-full">{loading ? "Oluşturuluyor..." : "Hesabı oluştur"}</button>
      </form>
    </Modal>
  </>;
}
