...
const kinds=["POSITIVE","NEGATIVE","NEUTRAL","QUESTION","COMPLAINT","SUGGESTION","SPAM"] as const;
const schema:Schema={type:SchemaType.ARRAY,items:{type:SchemaType.OBJECT,properties:{
...
},required:["id","kind","sentimentScore","confidence","topic","clusterKey","summary","suggestedReply"]}};

type Analysis={id:string;kind:(typeof kinds)[number];sentimentScore:number;confidence:number;topic:string;clusterKey:string;summary:string;suggestedReply:string};

const sleep=(ms:number)=>new Promise(resolve=>setTimeout(resolve,ms));
const transient=(error:unknown)=>/\b(429|500|502|503|504)\b|high demand|temporar|timeout|fetch failed|ECONNRESET/i.test(String(error));
...
async function generateWithRetry(client:GoogleGenerativeAI,prompt:string){
  ...
  for(let attempt=0;attempt<6;attempt++){
    ...
    try{
      return await client.getGenerativeModel({model,generationConfig:{responseMimeType:"application/json",responseSchema:schema,temperature:0.25}}).generateContent(prompt);
    }catch(error){
      lastError=error;
      if(!transient(error))throw error;
      const delay=Math.min(30_000,2_000*Math.pow(2,attempt))+Math.floor(Math.random()*750);
      await sleep(delay);
    }
  }
  throw lastError;
...
export async function analyzeComments(commentIds:string[]){
  const key=process.env.GEMINI_API_KEY;if(!key)throw new Error("GEMINI_API_KEY tanımlı değil.");
  const comments=await prisma.comment.findMany({where:{id:{in:commentIds}},select:{id:true,text:true,authorName:true,video:{select:{title:true,channel:{select:{name:true}}}}}});
...
- Her yorumun dilini kendin tespit et. suggestedReply mutlaka yorumla aynı dilde olsun. İspanyolca yoruma İspanyolca, Arapça yoruma Arapça, İngilizce yoruma İngilizce, Türkçe yoruma Türkçe cevap ver.
...
Veri:\n${JSON.stringify(comments)}`;
  ...
  const valid=rows.filter(row=>allowed.has(row.id)&&kinds.includes(row.kind));
  await prisma.$transaction(valid.map(row=>prisma.comment.update({where:{id:row.id},data:{
    kind:row.kind,
