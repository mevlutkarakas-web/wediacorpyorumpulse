  ...
  return <div className="mx-auto max-w-[1600px] space-y-6">
    ...
    {channels.length ? <section className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{channels.map(channel => {
      ...
      return <article key={channel.id} className="card p-5">
        <div className="flex items-start gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-xl bg-blue-600 font-bold text-white">f</span><div className="min-w-0 flex-1"><h2 className="truncate font-bold">{channel.versionChannel || channel.name}</h2><p className="truncate text-xs text-slate-400">{channel.name}</p></div><span className={`tag ${channel.facebookPageId ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300" : "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300"}`}>{channel.facebookPageId ? "Bağlı" : "Bekliyor"}</span></div>
