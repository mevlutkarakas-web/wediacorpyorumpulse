...
export default async function AnalyticsPage() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="card p-6"><div className="flex items-center gap-2"><Tags className="text-violet-500"/><h2 className="font-bold">En çok konuşulan konular</h2></div>{topics.length ? <div className="mt-5 space-y-4">{topics.map((item, index) => <div key={item.topic}><div className="flex justify-between text-sm"><b>{item.topic}</b><span className="text-slate-400">{item._count.toLocaleString("tr-TR")} yorum</span></div><div className="mt-2 h-2 rounded-full bg-muted"><div className="h-full rounded-full bg-violet-500" style={{ width: `${Math.max(5, 100 - index * 9)}%` }}/></div></div>)}</div> : <EmptyState title="Analiz sonuçları hazırlanıyor."/>}</section>
      <section className="card p-6"><div className="flex items-center gap-2"><MessageCircleQuestion className="text-cyan-500"/><h2 className="font-bold">Son AI değerlendirmeleri</h2></div>{recent.length ? <div className="mt-5 space-y-3">{recent.map(item => { const link = item.permalinkUrl || item.video.permalinkUrl; return <article className="rounded-xl border p-4" key={item.id}><div className="flex flex-wrap gap-2"><span className={`tag ${platformTagClass(item.platform)}`}>{platformLabel(item.platform)}</span><span className="tag bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400">{labels[item.kind]}</span>{item.topic && <span className="tag bg-muted">{item.topic}</span>}</div><p className="mt-3 text-sm">{item.aiSummary}</p><div className="mt-3 rounded-lg bg-muted/60 p-3 text-xs"><p><span className="text-slate-400">Kanal:</span> <b>{item.video.channel.versionChannel || item.video.channel.name}</b></p><p className="mt-1"><span className="text-slate-400">Video:</span> <b>{item.video.title}</b></p></div>{link && <a href={link} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-violet-600">Yoruma git <ExternalLink size={12}/></a>}</article>; })}</div> : <EmptyState icon={BrainCircuit} title="Gemini kuyruğu başladığında sonuçlar burada görünecek."/>}</section>
    </div>
  </div>;
}
